...
$keyword = isset($_GET['query']) ? strtolower(trim($_GET['query'])) : '';
$gender_filter = isset($_GET['gender']) ? $_GET['gender'] : '';
$concentration_filter = isset($_GET['concentration']) ? $_GET['concentration'] : '';
$min_price = isset($_GET['min_price']) ? (float) $_GET['min_price'] : 0;
$max_price = isset($_GET['max_price']) ? (float) $_GET['max_price'] : 1000; // Default max price
$sort_by = isset($_GET['sort']) ? $_GET['sort'] : 'relevance';
$scent_types = isset($_GET['scent_types']) ? $_GET['scent_types'] : [];

// Special handling for gender-specific search terms
if ($keyword == 'man' || $keyword == 'men' || $keyword == 'male') {
    // If searching for men's products, set gender filter automatically
    $gender_filter = 'male';
    // Don't use the keyword for general search to avoid matching "woman"/"women"
    $use_keyword_for_general_search = false;
} else if ($keyword == 'woman' || $keyword == 'women' || $keyword == 'female') {
    // If searching for women's products, set gender filter automatically
    $gender_filter = 'female';
    // Don't use the keyword for general search
    $use_keyword_for_general_search = false;
} else if ($keyword == 'unisex') {
    $gender_filter = 'unisex';
    $use_keyword_for_general_search = false;
} else {
    $use_keyword_for_general_search = true;
    
    // Check if searching for a common scent type
    $common_scents = ['citrus', 'floral', 'woody', 'spicy', 'oriental', 'fresh', 'sweet', 'fruity'];
    $is_known_scent = in_array(strtolower($keyword), $common_scents);
    
    // Check database for matching scent - more reliable than hardcoded list
    $scent_check_stmt = $conn->prepare("SELECT COUNT(*) as count FROM scents WHERE LOWER(ScentName) LIKE ?");
    $scent_param = '%' . strtolower($keyword) . '%';
    $scent_check_stmt->bind_param("s", $scent_param);
    $scent_check_stmt->execute();
    $scent_count = $scent_check_stmt->get_result()->fetch_assoc()['count'];
    
    // If this is a scent search, set a flag to modify our query later
    if ($is_known_scent || $scent_count > 0) {
        $scent_search = true;
        $debug_params[] = "Detected as scent search: " . $keyword . " (Count: " . $scent_count . ")";
    } else {
        $scent_search = false;
    }
}
...
// Check if the search query matches a scent name
if (!empty($keyword)) {
    $scent_check_stmt = $conn->prepare("SELECT ScentID FROM scents WHERE LOWER(ScentName) = LOWER(?)");
    $scent_check_stmt->bind_param("s", $keyword);
    $scent_check_stmt->execute();
    $scent_result = $scent_check_stmt->get_result();
    
    if ($scent_result->num_rows > 0) {
        $scent_row = $scent_result->fetch_assoc();
        // Add the scent ID to the scent_types array if it's not already there
        if (!in_array($scent_row['ScentID'], $scent_types)) {
            $scent_types[] = $scent_row['ScentID'];
        }
    }
    $scent_check_stmt->close();
}
...
// Append dynamic keyword search
if (!empty($keyword) && $use_keyword_for_general_search) {
    $keyword_param = '%' . $conn->real_escape_string($keyword) . '%';
    if (isset($scent_search) && $scent_search) {
        // For scent searches, search only in the scent field
        $where_conditions[] = "LOWER(s.ScentName) LIKE ?";
        $params[] = $keyword_param;
        $types .= "s";
        $debug_params[] = "Scent search condition: " . $keyword_param;
    } else {
        $where_conditions[] = "(
            LOWER(p.product_name) LIKE ? OR
            LOWER(p.product_description) LIKE ? OR
            LOWER(s.ScentName) LIKE ? OR
            LOWER(c.ConcentrationName) LIKE ? OR
            LOWER(p.gender) LIKE ?
        )";
        $params = array_merge($params, [$keyword_param, $keyword_param, $keyword_param, $keyword_param, $keyword_param]);
        $types .= "sssss";
        $debug_params[] = "General search condition: " . $keyword_param;
    }
}
...
if (!empty($gender_filter)) {
    ...
    $types .= "s";
    $debug_params[] = "Gender filter condition: " . $gender_db_value;
}
...
if (!empty($concentration_filter)) {
    ...
    $types .= "i";
    $debug_params[] = "Concentration filter condition: " . $concentration_filter;
}
...
if (!empty($scent_types)) {
    ...
    foreach ($scent_types as $scent_id) {
        $params[] = $scent_id;
        $types .= "i";
        $debug_params[] = "Scent type filter condition: " . $scent_id;
    }
    // Add count parameter for HAVING clause
    $params[] = count($scent_types);
    $types .= "i";
    $debug_params[] = "Scent type count condition: " . count($scent_types);
}
...
if ($sort_by === 'promotion') {
    ...
    $debug_params[] = "Promotion filter applied for sorting";
}
...
// Now we can safely use $sql for debugging
$debug_sql = $sql;

// Prepare statement
$stmt = $conn->prepare($sql);

// Debug SQL for developers (can be commented out in production)
if (isset($_GET['debug']) && $_GET['debug'] === 'true') {
    echo "<pre style='background:#f5f5f5;padding:15px;border:1px solid #ddd;margin:15px;'>";
    echo "SQL Query: " . htmlspecialchars($sql) . "<br><br>";
    echo "Parameters (Types: " . htmlspecialchars($types) . "):<br>";
    print_r($params);
    echo "<br><br>Debug Params:<br>";
    print_r($debug_params);
    echo "<br><br>Number of results: " . $total_results; // Note: total_results is not available here yet
    echo "</pre>";
}
...
if (strlen($types) !== count($params)) {
    die("Parameter count mismatch: Expected " . strlen($types) . ", got " . count($params) . ". Types: " . $types . ", Params: " . print_r($params, true) . " Debug Params: " . print_r($debug_params, true));
}
...
if (!$result) {
    die("Query execution failed: " . $conn->error . " SQL: " . htmlspecialchars($sql) . " Types: " . htmlspecialchars($types) . " Params: " . print_r($params, true));
}

// Debug SQL after execution for total_results
if (isset($_GET['debug']) && $_GET['debug'] === 'true') {
     echo "<pre style='background:#f5f5f5;padding:15px;border:1px solid #ddd;margin:15px;'>";
     echo "Total Results After Query: " . $total_results;
     echo "</pre>";
}

// Get scent types for filter
$scent_query = "SELECT DISTINCT s.ScentID, s.ScentName FROM scents s 
                JOIN product_scent ps ON s.ScentID = ps.scent_id 
                ORDER BY s.ScentName";
$scent_result = $conn->query($scent_query);
$scents = [];
if ($scent_result->num_rows > 0) {
    while ($row = $scent_result->fetch_assoc()) {
        $scents[] = $row;
    }
}
...
<head>
    ...
    <title>Search Results - ScentMatch</title>
        ...
        <div class="all-products-container">
            ...
            <div class="filter-sidebar">
                <form method="get" action="search.php">
                    <!-- Pass the search query to maintain it -->
                    <input type="hidden" name="query" value="<?php echo htmlspecialchars($keyword); ?>">
                    <div class="filter-card">
                            ...
                            <div class="scent-checkboxes">
                                ...
                                if ($scent_result->num_rows > 0) {
                                    while ($scent = $scent_result->fetch_assoc()) {
                                        ...
                                        echo '<input type="checkbox" name="scent_types[]" id="scent-' . $scent['ScentID'] . '" value="' . $scent['ScentID'] . '" class="scent-checkbox"' . 
                                             ($is_checked ? ' checked' : '') . '>';
                        ...
                        <div class="filter-actions">
                            ...
                            <a href="search.php?query=<?php echo urlencode($keyword); ?>" class="filter-button clear-filter">Clear</a>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <div class="search-header">
                    <div class="search-result-count">
                        <?php if (!empty($keyword)): ?>
                            <?php if (!$use_keyword_for_general_search): ?>
                                <?php
                                    $gender_display = '';
                            ...
                            <?php elseif (isset($scent_search) && $scent_search && !empty($gender_filter)): ?>
                                <?php
                                    $gender_display = '';
                                    if ($gender_filter === 'male') $gender_display = "Men's";
                                    elseif ($gender_filter === 'female') $gender_display = "Women's";
                                    elseif ($gender_filter === 'unisex') $gender_display = "Unisex";
                                ?>
                                <h2 style="margin-top: 0; margin-bottom: 5px;"><?php echo htmlspecialchars($gender_display); ?> Products with "<?php echo htmlspecialchars(ucfirst($keyword)); ?>" Scent</h2>
                            <?php elseif (isset($scent_search) && $scent_search): ?>
                                <h2 style="margin-top: 0; margin-bottom: 5px;">Products with "<?php echo htmlspecialchars(ucfirst($keyword)); ?>" Scent</h2>
                            <?php else: ?>
                                <h2 style="margin-top: 0; margin-bottom: 5px;">Search Results for "<?php echo htmlspecialchars($keyword); ?>"</h2>
                            <?php endif; ?>
                        <?php else: ?>
                ...
                <div class="container">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            echo '</div>';
                            echo '<p class="total-sales">Sold ' . htmlspecialchars($row['total_sold']) . '</p>';
                            echo '</div>';
                            
                            // Add compare container
                            echo '<div class="compare-container" onclick="event.stopPropagation();">';
                            echo '<input type="checkbox" id="compare_search_' . $row['product_id'] . '" class="compare-checkbox" data-product-id="' . $row['product_id'] . '">';
                            echo '<label for="compare_search_' . $row['product_id'] . '" class="compare-label">Compare</label>';
                            echo '</div>';
                        ...
                        echo '<p>Try adjusting your search criteria or browse our categories.</p>';
                        echo '</div>';
                    }
                    ?>
                </div>
